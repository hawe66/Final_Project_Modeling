data {
  ...
  real<lower=0, upper=1> result[N,T];   // result
...
transformed parameters{
  ...
  for (i in 1:N) {
    ...
    alpha[i] = Phi_approx( mu_a  + sigma_a * alpha_pr[i] ) * 2 - 1;
    theta[i] = Phi_approx( mu_t  + sigma_t * theta_pr[i] ) * 2 - 1;
...
for (i in 1:N) {
    ...
    for (t in 1:T) {
      ...
      PEr = fabs(result[i, t] - binomial_cdf(free, 4, gamma[t]));
      gamma[t+1] = gamma[t] + inv_logit(alpha[i]+theta[i]*PEr) * PEs;
